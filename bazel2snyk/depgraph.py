...
class DepGraph(object):
    def __init__(
        ...
        self.dep_graph = {
            "depGraph": {
                "schemaVersion": "1.2.0",
                "pkgManager": {"name": self.pkg_manager_name},
                "pkgs": [
                    {"id": "app@1.0.0", "info": {"name": "app", "version": "1.0.0"}}
                ],
                "graph": {
                    "rootNodeId": "root-node",
                    "nodes": [
                        {"nodeId": "root-node", "pkgId": "app@1.0.0", "deps": []}
                    ],
                },
            }
        }
    ...
    def add_dep(self, child_node_id: str, parent_node_id: str = None):
        ...
        if parent_node:
            if child_node_id:
                ...
                dep_entry = {"nodeId": child_node_id}
    ...
    def remove_dep(self, child_node_id: str, parent_node_id: str = None):
        ...
        for subtree_node in graph_subtree:
            ...
            child_node_entry = {"nodeId": f"{child_node_id}"}
    ...
    def rename_depgraph(self, new_name):
        ...
        old_package_name, package_version = self.dep_graph["depGraph"]["graph"][
            "nodes"
        ][root_node_index]["pkgId"].split("@")

        # Rename the root note
        self.dep_graph["depGraph"]["graph"]["nodes"][root_node_index]["nodeId"] = (
            new_name
        )
        self.dep_graph["depGraph"]["graph"]["nodes"][root_node_index]["pkgId"] = (
            f"{new_name}@{package_version}"
        )
        ...
        self.dep_graph["depGraph"]["pkgs"][target_package_index]["id"] = (
            f"{new_name}@{package_version}"
        )
        self.dep_graph["depGraph"]["pkgs"][target_package_index]["info"]["name"] = (
            new_name
        )
